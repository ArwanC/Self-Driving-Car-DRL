import numpy as np
from gym_game.envs.geometry import Point, Rectangle, Circle, Ring, Line
...
class SensorEntity(Entity):
    def __init__(self, p1: Point, p2: Point, range_sensors: int,  center: Point=Point(0,0), heading: float=0, color: str = 'green'):
    ...
    def buildGeometry(self):
        ...
        for i in range(self.precision):
            self.obj.append(Line(Point(self.list_points[i][0], self.list_points[i][1]),Point(self.list_points[i+1][0], self.list_points[i+1][1])))

class TextEntity(Entity):
    def __init__(self, anchor: Point, text: str, center: Point=Point(0,0), heading: float=0, color: str = "black"):
