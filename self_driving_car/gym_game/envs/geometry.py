...
class Line:
    ...
    def intersectsWith(self, other: Union['Line','Rectangle','Circle','Ring']):
        if isinstance(other, Line):
            ...
            return other.m.distanceTo(self.p2) <= other.r
            
        elif isinstance(other, Ring):
            # return (other.m.distanceTo(self.p1) <= other.r_inner or other.m.distanceTo(self.p2) <= other.r_inner) and other.m.distanceTo(self.p1) < other.r_outer
            return (other.m.distanceTo(self.p2) >= other.r_inner and other.m.distanceTo(self.p2) <= other.r_outer) #considering the car cant be inside the building
        raise NotImplementedError
...
class Text:
    def __init__(self, m: Point, text: str):
        self.m = m
        self.text = text

    def __str__(self):
        return 'Text(' + str(self.m) + ', text = ' + self.text + ')'
